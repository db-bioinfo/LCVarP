...
def check_PP3(line,Funcanno_flgs,Allels_flgs):
    '''
    ENHANCEMENT 3,4,5: Enhanced PP3 with calibrated predictors, splicing, and conservation
    Multiple lines of computational evidence support a deleterious effect
    '''
    PP3 = 0
    cls = line.split('\t')
    
    # ENHANCEMENT 3: Calibrated computational predictors (ClinGen recommendations)
    # Priority 1: MetaRNN (most accurate)
    try:
    ...
    try:
        if 'BayesDel_addAF_score' in Funcanno_flgs and cls[Funcanno_flgs['BayesDel_addAF_score']] != '.':
            ...
            if bayesdel_score >= 0.421:
                return 4  # Strong
            elif bayesdel_score >= 0.219:
                return 2  # Moderate
            elif bayesdel_score >= 0.16:
                return 1  # Supporting
    except (ValueError, KeyError):
        pass
    
    # Priority 4: AlphaMissense (newest predictor)
    try:
        if 'AlphaMissense_score' in Funcanno_flgs and cls[Funcanno_flgs['AlphaMissense_score']] != '.':
            ...
            if alphamissense_score >= 0.994:
                return 4  # Strong
            elif alphamissense_score >= 0.956:
                return 2  # Moderate
            elif alphamissense_score >= 0.787:
                return 1  # Supporting
    except (ValueError, KeyError):
        pass
    
    # ENHANCEMENT 4: Enhanced splicing prediction (high priority)
    try:
        ada_score = cls[Funcanno_flgs.get('dbscSNV_ADA_SCORE', -1)]
        rf_score = cls[Funcanno_flgs.get('dbscSNV_RF_SCORE', -1)]
    ...
    # ENHANCEMENT 5: Conservation as fallback (if no other predictors available)
    try:
    ...
    # Alternative conservation: phyloP
    try:
        if 'phyloP100way_vertebrate' in Funcanno_flgs and cls[Funcanno_flgs['phyloP100way_vertebrate']] != '.':
            ...
            if phylop_score >= 7.52:  # High conservation
                return 1  # Supporting pathogenic
    except (ValueError, KeyError):
        pass
...
def check_BP4(line,Funcanno_flgs,Allels_flgs):
    '''
    ENHANCEMENT 3,5: Enhanced BP4 with calibrated predictors and conservation
    ...
    # ENHANCEMENT 3: Calibrated computational predictors for benign evidence
    # Priority 1: MetaRNN (most accurate)
    try:
        if 'MetaRNN_score' in Funcanno_flgs and cls[Funcanno_flgs['MetaRNN_score']] != '.':
            metarnn_score = float(cls[Funcanno_flgs['MetaRNN_score']])
    ...
    try:
        if 'BayesDel_addAF_score' in Funcanno_flgs and cls[Funcanno_flgs['BayesDel_addAF_score']] != '.':
            bayesdel_score = float(cls[Funcanno_flgs['BayesDel_addAF_score']])
            if bayesdel_score <= -0.238:
                return 4  # Strong benign
            elif bayesdel_score <= -0.0746:
                return 2  # Moderate benign
            elif bayesdel_score <= -0.00775:
                return 1  # Supporting benign
    except (ValueError, KeyError):
        pass
    ...
    try:
        if 'AlphaMissense_score' in Funcanno_flgs and cls[Funcanno_flgs['AlphaMissense_score']] != '.':
            ...
            if "missense" in line_tmp.lower() or "nonsynon" in line_tmp.lower():
                alphamissense_score = float(cls[Funcanno_flgs['AlphaMissense_score']])
                if alphamissense_score <= 0.0853:
                    return 4  # Strong benign
                elif alphamissense_score <= 0.166:
                    return 2  # Moderate benign
                elif alphamissense_score <= 0.316:
                    return 1  # Supporting benign
    except (ValueError, KeyError):
        pass
    
    # ENHANCEMENT 5: Conservation for benign evidence (low conservation = benign)
    # FIXED: Remove aggressive fallback logic for missing data
    try:
        if 'GERP++_RS' in Funcanno_flgs and cls[Funcanno_flgs['GERP++_RS']] != '.':
            gerp_score = float(cls[Funcanno_flgs['GERP++_RS']])
            if gerp_score <= -1.04:
                ...
                return 1  # Supporting benign
        # REMOVED: Overly aggressive fallback for missing conservation
    except (ValueError, KeyError):
        pass
    
    # Check splicing - no predicted impact (FIXED: More conservative approach)
    try:
        ada_score = cls[Funcanno_flgs.get('dbscSNV_ADA_SCORE', -1)]
        rf_score = cls[Funcanno_flgs.get('dbscSNV_RF_SCORE', -1)]
        ...
        if ada_score != '.' and rf_score != '.':
            ...
            if ada_val < 0.958 and rf_val < 0.584:
                return 1  # Supporting benign
        # REMOVED: Aggressive fallback for missing splicing data
